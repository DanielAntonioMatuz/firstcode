...
export class LibraryWordReservedService {
  ...
  constructor() {
    this.lexicoUpwind = ['args', 'body', 'case', 'char', 'displayEnter', 'displayView', 'float', 'for', 'if', 'mult', 'rest', 'run',
      'string', 'sum', 'int', 'boolean'];
  }
  ...
  validarPalabra(cadena) {
    return cadena === ('args') || cadena === ('body') || cadena === ('case') || cadena === ('catch') ||
      ...
      cadena === ('final') || cadena === ('float') || cadena === ('for') ||
      cadena === ('if') || cadena === ('mult') || cadena === ('rest') || cadena === ('run') ||
      cadena === ('string') || cadena === ('sum') || cadena === ('try') || cadena === ('int') ||
      cadena === ('boolean');
  }

  predictivoUpwind(value) {
    let index = [];
    for (let i = 0; i < this.lexicoUpwind.length; i++) {
      if (this.lexicoUpwind[i].includes(value)) {
        index.push(this.lexicoUpwind[i]);
      }
    }
    return index;
  }
  ...
  regresarSignificado(cadena) {
    switch (cadena) {
      ...
      case 'displayEnter':
        return 'Funcion de entrada de datos a consola';
      case 'displayView':
        return 'Funcion de salida de datos a consola';
      case 'if':
      case 'for':
